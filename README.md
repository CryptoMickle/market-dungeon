...
7. Inspect the generated 1200×675 run card. In the newer source, **Challenge a player** shares a text-and-link invitation through the device share menu (or offers copy/manual text); **Share on X** directly opens a text-only X draft; **Save card** opens the prepared image and save/share options. These are three distinct actions. Save the PNG first to attach it manually to the X draft; the web link cannot attach a local image. A PNG download goes to the browser's download location, not automatically to iPhone Photos. The challenge link opens a fresh, separately sealed replay on the same network profile. Final physical iPhone acceptance and a Production release of this new flow remain pending. In mainnet only, **Continue on dreamDEX** opens the external current dreamDEX market.
