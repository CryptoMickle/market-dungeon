...
export function createJudgeReplayRevealHandler(input: {
  profile: JudgeNetworkProfile;
  dedupe: RevealStore;
  rateNamespace: string;
}) {
  const rateConfig = { namespace: input.rateNamespace, limit: 12, windowMs: 60_000 };
  ...
  return async function POST(request: Request) {
    ...
    const deduped = await input.dedupe({
      commitment,
      digest,
      expiresAt: claims.expiresAt * 1_000,
      verify: async (): Promise<RevealResult> => {
        try {
          const rawMarket = await fetchFullMarket(claims.marketId, input.profile);
          ...
          const hydrated = await hydrateMarket(rawMarket, true, input.profile);
          return {
            status: 200,
            body: {
              ...hydrated,
              ...
              replayProof: {
                ...
                ...(input.profile.id === 'shannon-testnet'
                  ? { profileId: input.profile.id, chainId: input.profile.chainId }
                  : {}),
              },
              combatProof: {
                verified: true,
          ...
          if (isRetryableUpstreamError(error)) {
            return {
              ...
              body: { error: 'Somnia settlement services are temporarily unavailable. Retry this reveal.', retryState: 'upstream_retry', retryAfter: error.retryAfter },
            };
          }
