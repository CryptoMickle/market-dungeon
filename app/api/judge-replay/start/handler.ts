...
import type { CandidateData, ReplayCandidateData } from './state.ts';
...
) => Promise<{ data: CandidateData; cacheState: 'hit' | 'shared' | 'miss' }>;
...
export function createJudgeReplayStartHandler(input: {
  ...
  return async function POST(request: Request) {
    ...
    try {
      ...
      const publicMarket = input.profile.id === 'somnia-mainnet'
        ? { asset: 'BTC', intervalSec: replayPool.intervalSec, network: input.profile.name, chainId: input.profile.chainId }
        : { asset: 'BTC', intervalSec: replayPool.intervalSec, network: input.profile.name, chainId: input.profile.chainId, profileId: input.profile.id };
