...
The application has no account system, wallet connection, approval, order, or transaction flow. Full Expedition progress, persistent gold, next-run potion count and the sound preference are stored only in the player's browser. During post-reveal verification, the browser contacts the fixed public Somnia mainnet or Shannon testnet RPC directly to reproduce the displayed proof. The separate `/verify`, `/shannon/verify` and `/shannon/live-judge/verify` routes parse their corresponding exported proof JSON locally and never uploads the complete file. Its public-key GET contains no proof data; its fixed Somnia RPC requests contain only the artifact's public block hash/reference, contract targets, and two read-only calldata inputs needed to reproduce the recorded state. The verification result is not sent to analytics. Continuing to dreamDEX opens that separate service in a new tab. See [Vercel's Web Analytics privacy documentation](https://vercel.com/docs/analytics/privacy-policy).
