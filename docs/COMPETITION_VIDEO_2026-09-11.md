...
One recorded Live Judge round supplies the prediction lock, both fights, the settled result, the independent verifier and the exported card. The player chooses BTC UP against a $77,213.05 target, wins combat and receives a verified BTC DOWN result. The boss returns for its final strike. The verifier reports PASS for that losing run: the evidence matches; the prediction did not win.

Separate labeled excerpts show Full Expedition, Historical Replay and owner-recorded iPhone play. The film also introduces the dungeon’s original character art and humor. It is an edited demonstration, not a continuous two-minute completion benchmark, a completed 40-room run or an additional user study.

The **Continue on dreamDEX** destination is explicitly a separate five-minute mainnet market. No order is placed, and the testnet result is not transferred to that market.
...
Gameplay was recorded by the project owner. Character interludes reuse the original project artwork; readable proof and card details use the actual captured result and original card export. Editing and narration use AI assistance, including the Marin voice. The film uses the game’s original procedural sounds and boss music. It does not use the licensed music track from the older baseline video. See [provenance and AI disclosure](PROVENANCE_AND_PRIVACY.md).
